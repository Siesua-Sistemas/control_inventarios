...
function EntregarPanel({
  ...
  return (
    <div className="mb-6 rounded-2xl border border-indigo-900/50 bg-slate-900 p-6">
      <div className="mb-5 flex items-center justify-between">
        <h2 className="text-base font-semibold">Registrar entrega</h2>
        <button onClick={onClose} className="text-slate-500 hover:text-slate-300 text-sm">✕ Cancelar</button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            ...
            />
            {sugeridos.length > 0 && (
              <ul className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-slate-700 bg-slate-950 shadow-xl">
                {sugeridos.slice(0, 10).map((eq) => (
                  <li key={eq.id}>
                    <button type="button" onClick={() => { setCart((p) => [...p, eq]); setEqSearch(''); }}
                      ...
                      <span className="font-mono text-xs text-cyan-400 w-20 shrink-0">{eq.codigo_interno}</span>
                      <span className="font-medium text-slate-200">{eq.marca} {eq.modelo}</span>
          ...
          {cart.length > 0 && (
            <ul className="mt-2 space-y-1.5">
              {cart.map((eq) => (
                <li key={eq.id} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800 px-3 py-2">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="font-mono text-xs text-cyan-400 shrink-0">{eq.codigo_interno}</span>
                    <span className="text-sm text-slate-200 truncate">{eq.marca} {eq.modelo}</span>
                    <span className="text-xs text-slate-500 shrink-0">{eq.tipo}</span>
                  </div>
                  <button type="button" onClick={() => setCart((p) => p.filter((x) => x.id !== eq.id))}
                    className="ml-3 shrink-0 text-xs text-slate-500 hover:text-red-400">✕</button>
                </li>
              ))}
            </ul>
          )}
          {!cart.length && !eqSearch && (
            <p className="mt-1.5 text-xs text-slate-600">Escribe para buscar equipos disponibles.</p>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3">
          <button type="button" onClick={onClose} className="text-sm text-slate-500 hover:text-slate-300">Cancelar</button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        ...
        <p className="mb-5 text-sm text-slate-400">
          <span className="font-mono text-cyan-400">{asignacion.equipment_codigo}</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 rounded-lg border border-slate-700 py-2 text-sm text-slate-300 hover:bg-slate-800 transition-colors">
              Cancelar
            </button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-400">Equipo *</label>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-400">Bodega destino *</label>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-400">Observaciones</label>
            <input value={obs} onChange={(e) => setObs(e.target.value)} placeholder="Opcional"
              className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none" />
          </div>
          {error && <p className="text-sm text-red-300">{error}</p>}
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-400">{activas.length} equipo{activas.length !== 1 ? 's' : ''} actualmente asignado{activas.length !== 1 ? 's' : ''}</p>
          </div>
        ...
        {exito && (
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-800/50 bg-emerald-500/10 px-5 py-4">
            <div>
              <p className="font-semibold text-emerald-300">Entrega registrada</p>
              <p className="text-sm text-slate-400">{exito.eqIds.length} equipo{exito.eqIds.length !== 1 ? 's' : ''} asignado{exito.eqIds.length !== 1 ? 's' : ''} correctamente.</p>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => setExito(null)} className="text-sm text-slate-500 hover:text-slate-300">
                Descartar
              </button>
            </div>
          </div>
        ...
        <div className="mb-4 flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none"
          />
          {search && (
            <button onClick={() => setSearch('')} className="rounded-lg border border-slate-700 px-3 py-2 text-xs text-slate-400 hover:text-slate-200">
              ✕ Limpiar
            </button>
          )}
          {search && (
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-950 text-xs uppercase tracking-wider text-slate-400">
              <tr>
                ...
                <th className="hidden md:table-cell px-4 py-3">Fecha</th>
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={a.id} className="border-t border-slate-800 hover:bg-slate-800/30 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-sm text-slate-200">{a.equipment_marca} {a.equipment_modelo}</p>
                    <p className="text-xs text-slate-500">{a.equipment_tipo}</p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm font-medium text-slate-200">{a.empleado_nombre ?? '—'}</p>
                    {a.empleado_cedula && <p className="text-xs text-slate-500">{a.empleado_cedula}</p>}
                  </td>
                  <td className="hidden md:table-cell px-4 py-3 text-sm text-slate-400">{a.equipment_sede}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1.5 items-end md:flex-row md:flex-wrap md:justify-end">
                      <button
                        ...
                        className="w-full md:w-auto rounded-md border border-amber-700/60 bg-amber-500/10 px-3 py-1.5 text-xs font-medium text-amber-300 hover:bg-amber-500/20 transition-colors text-center"
                      >
                        Devolver
                      </button>
                      <Link
                        href={`/asignaciones/devolucion?eq=${a.equipment_id}`}
                        className="w-full md:w-auto rounded-md border border-lime-800/60 bg-lime-900/20 px-3 py-1.5 text-xs font-medium text-lime-400 hover:bg-lime-900/40 transition-colors text-center"
                      >
                        Devolver con acta
                      </Link>
                      {actasMap.get(a.equipment_id) ? (
                        <Link
                          ...
                          className="w-full md:w-auto rounded-md border border-lime-800/60 bg-lime-900/30 px-3 py-1.5 text-xs font-semibold text-lime-400 hover:bg-lime-900/50 transition-colors text-center"
                        >
                          ✓ Ver acta firmada
                        </Link>
                      ) : (
                        <Link
                          ...
                          className="w-full md:w-auto rounded-md border border-indigo-800/60 bg-indigo-600/20 px-3 py-1.5 text-xs font-medium text-indigo-300 hover:bg-indigo-600/40 transition-colors text-center"
                        >
                          Acta entrega
                        </Link>
                      )}
