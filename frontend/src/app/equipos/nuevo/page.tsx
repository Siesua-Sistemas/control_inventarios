...
const EMPTY: EquipmentPayload = {
  ...
  estado: 'Disponible', criticidad: 'Media', dominio: 'IT',
...
export default function NuevoEquipoPage() {
  ...
  useEffect(() => {
    if (tipos.length && !tipos.some((t) => t.nombre === form.tipo)) {
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        <div className="mb-6">
          ...
          <h1 className="mt-1 text-3xl font-bold text-slate-900 dark:text-slate-100">Nuevo equipo</h1>
        </div>
              ...
              <div className={`${fieldClass} sm:col-span-2`}>
                ...
                {selectedBodega && (
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Sede: <span className="font-medium text-slate-700 dark:text-slate-300">{selectedBodega.sede}</span>
                  </p>
                )}
              </div>
              <div className={fieldClass}>
