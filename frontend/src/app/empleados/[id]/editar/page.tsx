...
import { SedeJornadaOut, getEmpleado, getSedesJornada, isAuthenticated, updateEmpleado } from '@/lib/api';
...
export default function EditarEmpleadoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getEmpleado(id), getSedesJornada()])
      .then(([emp, sedes]) => {
        setForm({
          nombres: emp.nombres, apellidos: emp.apellidos, cedula: emp.cedula,
          cargo: emp.cargo ?? '', departamento: emp.departamento ?? '',
          sede: emp.sede ?? '', email: emp.email ?? '', telefono: emp.telefono ?? '',
        });
        setEnJornada(emp.en_jornada);
        setSedesJornada(emp.sedes_jornada_ids ?? []);
        setSedesDisp(sedes);
      })
      .catch((err) => setError(err instanceof Error ? err.message : 'No se pudo cargar el empleado'))
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        <div className="mb-6">
          ...
          <h1 className="mt-1 text-3xl font-bold text-slate-900 dark:text-slate-100">Editar empleado</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input id="email" className="w-full" type="email" value={form.email} onChange={set('email')} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label htmlFor="telefono" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Teléfono</label>
              <input id="telefono" className="w-full" value={form.telefono} onChange={set('telefono')} />
            </div>
          </div>
