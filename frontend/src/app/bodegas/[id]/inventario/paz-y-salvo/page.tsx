...
export default function PazYSalvoBodegaPage() {
  ...
  if (step === 'listo' && actaId) return (
    <><NavBar /><main className="mx-auto max-w-xl px-4 py-16 text-center">
      ...
      <p className="mt-2 text-slate-400">La bodega fue confirmada en buen estado por ambas partes.</p>
  ...
  if (step === 'guardando') return (
    <><NavBar /><main className="flex min-h-screen items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-slate-400">Generando paz y salvo...</p>
      </div>
    </main></>
  );
    ...
    <><NavBar />
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400">{data.bodega.nombre} · {data.bodega.sede} · {data.total} equipos</p>
        </div>
            ...
            <div key={s.key} className="flex items-center">
              ...
              <div className={`rounded-full px-4 py-1.5 text-xs font-semibold transition-colors ${
                step === s.key ? 'bg-cyan-500 text-slate-950'
                  : step === 'firmas' && s.key === 'revision' ? 'bg-lime-900/40 text-lime-400'
          ...
          <div className="rounded-2xl border border-slate-700 overflow-hidden">
            <div className="bg-slate-900 px-6 py-4 border-b border-slate-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Confirma que todos los equipos están presentes y en buen estado</p>
              </div>
            ...
            <div className="divide-y divide-slate-800">
              {data.equipos.length === 0 ? (
                ...
                return (
                  <label key={eq.id}
                    className={`flex cursor-pointer items-center gap-4 px-6 py-4 transition-colors ${isChecked ? 'bg-lime-900/10' : 'hover:bg-slate-900'}`}>
                    ...
                    <div className="grid flex-1 grid-cols-4 gap-3 items-center min-w-0">
                      <div><p className="text-xs text-slate-500">Código</p>
                        <p className="font-mono text-sm font-bold text-cyan-400">{eq.codigo_interno}</p></div>
                      <div><p className="text-xs text-slate-500">Tipo</p>
                        <p className="text-sm text-slate-300">{eq.tipo}</p></div>
                      <div><p className="text-xs text-slate-500">Marca / Modelo</p>
                        <p className="text-sm font-semibold text-white">{eq.marca} {eq.modelo}</p></div>
                      <div>
        ...
        {step === 'firmas' && (
          <div className="space-y-6">
            <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
              <p className="mb-4 text-xs font-semibold uppercase tracking-widest text-slate-400">Responsables del paz y salvo</p>
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="flex flex-col gap-1.5">
                  <span className="text-xs text-slate-400">Responsable de la bodega (quien certifica) *</span>
                ...
                <label className="flex flex-col gap-1.5">
                  <span className="text-xs text-slate-400">Supervisor / Administrador (quien aprueba) *</span>
            ...
            {saveError && <p className="rounded-lg bg-red-900/30 px-4 py-2 text-sm text-red-300">{saveError}</p>}
