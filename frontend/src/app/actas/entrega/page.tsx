...
export default function ActaEntregaPage() {
  ...
  const [modo, setModo] = useState<'sede' | 'personal'>('sede');
  ...
  useEffect(() => {
    ...
    if (!authLoading && !canEntregar) { router.replace('/actas'); return; }
    Promise.all([
      listEquipment({ estado: 'Disponible' }),
      listEquipment({ estado: 'En bodega' }),
      listBodegas(),
    ]).then(([d, b, bod]) => {
      setEquiposDisponibles([...d.items, ...b.items]);
      setBodegas(bod.items);
    }).catch(() => null);
  }, [router, authLoading, canEntregar]);

  const sedes = useMemo(() => [...new Set(bodegas.map((b) => b.sede))].sort(), [bodegas]);

  // Auto-detectar bodega origen a partir de la sede del primer equipo del carrito
  useEffect(() => {
    if (cart.length === 0) { setBodegaOrigenId(''); return; }
    const match = bodegas.find((b) => b.sede === cart[0].sede);
  ...
  const destinoNombre = modo === 'sede' ? sedeDestino : empNombre;
  const canSubmit = cart.length > 0 && (modo === 'personal' ? empId !== null : sedeDestino !== '');
  ...
  const handleGuardar = async () => {
    ...
    if (!entregaNombre.trim()) { setError('Indica quién entrega.'); return; }
    setError('');
    setSaving(true);
    try {
      ...
      const acta = await createActaEntrega({
        tipo: 'asignacion',
        sede: modo === 'sede' ? sedeDestino : (cart[0]?.sede ?? ''),
        titulo: destinoNombre || 'Entrega',
        entrega_nombre: entregaNombre.trim(),
        recibe_nombre: destinoNombre || 'Destinatario',
    ...
    return (
      <>
        ...
        <main className="mx-auto max-w-xl px-4 py-16 text-center">
          ...
          <h1 className="text-2xl font-bold">Entrega registrada</h1>
          <p className="mt-2 text-slate-600 dark:text-slate-400">
            {cart.length} equipo{cart.length !== 1 ? 's' : ''} entregado{cart.length !== 1 ? 's' : ''} a {destinoNombre}.
          </p>
          <div className="mt-8 flex flex-col gap-3">
            <Link href={`/actas/${actaId}/imprimir`} className="rounded-lg bg-indigo-600 px-6 py-3 text-sm font-semibold text-white hover:bg-indigo-500 transition-colors">
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">Entrega equipos a una sede o a una persona, con firma de respaldo.</p>
        </div>
        ...
        <div className="mb-8 flex items-center gap-0">
          {[{ key: 'seleccion', label: '1. Equipos' }, { key: 'datos', label: '2. Datos y firmas' }].map((s, i) => (
            <div key={s.key} className="flex items-center gap-0">
              {i > 0 && <div className={`h-px w-12 ${step === 'datos' ? 'bg-indigo-500' : 'bg-slate-300 dark:bg-slate-700'}`} />}
        ...
        {step === 'seleccion' && (
          <div className="space-y-4">
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400">Tipo de entrega</label>
            ...
            {modo === 'sede' && (
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400">Sede destino *</label>
                <select value={sedeDestino} onChange={(e) => setSedeDestino(e.target.value)}
        ...
        {step === 'datos' && (
          <div className="space-y-6">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
              <p className="mb-4 text-xs font-semibold uppercase tracking-widest text-slate-600 dark:text-slate-400">Datos de la entrega</p>
              <div className="grid grid-cols-2 gap-4">
                <EmpleadoAutocomplete label="Quien entrega *" required value={entregaNombre} onChange={setEntregaNombre} placeholder="Buscar por nombre..." />
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-slate-600 dark:text-slate-400">Bodega origen</label>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
                <SignaturePad label="Firma quien recibe" name={destinoNombre || '—'} onChange={setFirmaRecibe} />
              </div>
            </div>
