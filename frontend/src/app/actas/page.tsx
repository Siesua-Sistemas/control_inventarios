      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-slate-400">Registro de todas las entregas formalizadas</p>
          </div>
          <span className="rounded-2xl bg-indigo-500/10 px-5 py-2 text-2xl font-bold text-indigo-400">{total}</span>
        </div>

        {/* Filtros */}
        <form onSubmit={handleFilter} className="mb-6 flex flex-wrap items-end gap-3 rounded-2xl border border-slate-800 bg-slate-900 px-5 py-4">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 py-16 text-center">
            <p className="text-slate-400">No hay actas registradas.</p>
          </div>
        ) : (
          <div className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-900">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {actas.map((acta) => (
                  <tr key={acta.id} className="border-t border-slate-800 hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${TIPO_STYLES[acta.tipo] ?? 'bg-slate-700 text-slate-300 border-slate-600'}`}>
                        {TIPO_LABEL[acta.tipo] ?? acta.tipo}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-medium text-white">{acta.titulo}</p>
                      <p className="text-xs text-slate-500">{acta.sede}</p>
                    </td>
                    <td className="px-4 py-3 text-slate-300">{acta.entrega_nombre}</td>
                    <td className="px-4 py-3 text-slate-300">{acta.recibe_nombre}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {acta.firma_entrega && acta.firma_recibe
                        ? <span className="text-emerald-400 text-xs font-semibold">✓ Completas</span>
                        : acta.firma_entrega || acta.firma_recibe
                        ? <span className="text-yellow-400 text-xs">Parcial</span>
                        : <span className="text-slate-600 text-xs">Sin firma</span>}
                    </td>
                    <td className="px-4 py-3">
                      <Link
